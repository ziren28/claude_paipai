...
发送 `/menu`、`/m`、`派派` 或 `pp` 进入菜单：

```
🤖 派派控制中心
━━━━━━━━━━━━━━━━
🟢 Claude: 空闲 | CPU 2% MEM 4%
接入: 官方直连
━━━━━━━━━━━━━━━━
1. 🔌 Claude 接入点
2. 🔧 服务管理
...
**快捷入口：** `派派1` / `pp2` / `/m4` 直达子菜单，不需要先进主菜单。

---

### 菜单 1：🔌 Claude 接入点

管理 Claude API 接入配置（官方直连 / 自建网关 / 第三方）：
...
- `s1` → 切换到官方直连，自动重启 Claude
- `+` → 新建预设（引导输入名称 → URL → API Key）
- `e2` → 编辑预设（修改名称/URL/Key）
...
━━━━━━━━━━━━━━━━
1. 查看状态
2. 重启 (保存会话)     ← Ctrl+C → claude -c
3. 中止当前任务        ← 发送 Ctrl+C
4. 启动               ← 在 tmux 中启动
5. 强制杀死并重启      ← pkill + 重启
6. 查看 Claude 日志    ← tmux 终端最近20行
7. 查看操作日志        ← 所有菜单操作记录
0. 返回主菜单
```

**使用场景：**
- Claude 卡住不动 → 按 `3` 中止当前任务
- Claude 彻底挂了 → 按 `5` 强制杀死并重启
- 想看 Claude 在干什么 → 按 `6` 查看终端输出
- 切换了接入点需要重启 → 按 `2` 保存会话重启
...
拦截列表：`rm -rf /`、`mkfs`、`dd if=`、`> /dev/sd`、`fork bomb`、`chmod -R 777 /`
